/* pm-agent — operator UI logic. Vanilla, read-only, offline.
   Fetches /api/*, renders a calm ledger, live-updates over SSE.
   The worktrees panel (right rail) lives in worktrees-panel.js. */

import {
  renderWorktreePanel, repaintPanel, handleWtAction, closeWtMenu,
  showBnameTip, hideBnameTip,
} from "./worktrees-panel.js";
...
// ---- app state (shared with worktrees-panel.js) -----------------------------
export const state = {
  view: "initiatives",     // initiatives | timeline | thread | usage
  filter: "unfinished",    // all | unfinished (Initiatives only)
  threadId: null,
  scrollToLoose: false,    // rail-foot click pending a scroll to the Loose ends section
  meta: null,
  ...
  // worktrees panel (right rail on Initiatives/Timeline; rendering lives in worktrees-panel.js)
  wt: {
...
// ---- routing ----------------------------------------------------------------
function parseHash(){
  const h = (location.hash || "#/initiatives").replace(/^#\/?/, "");
  ...
  // legacy routes from the Work/In-flight era — send old bookmarks to Initiatives
  if(parts[0] === "work" || parts[0] === "inflight") return { view:"initiatives", redirect:true };
  return { view:"initiatives" };
}

function syncNav(){
  // thread drill-in belongs to Initiatives; timeline + usage stand alone
  const activeNav = state.view === "usage" ? "usage"
    : state.view === "timeline" ? "timeline" : "initiatives";
...
async function onRoute(){
  const r = parseHash();
  if(r.redirect){ location.replace("#/initiatives"); return; } // hashchange re-fires onRoute
...
async function refresh(force){
  try{
    if(state.view === "thread")        await renderThread(force);
    else if(state.view === "usage")    await renderUsage(force);
    else if(state.view === "timeline") await renderTimeline(force);
    else                               await renderInitiatives(force);
  }catch(err){
...
async function renderThread(force){
  ...
  const genesis = th.genesis
    ...
    : "";
  // Forward-compat: `focus` (momentary focus, distinct from the durable goal) and `why` are
  // being added by a parallel branch — render them when present, disappear cleanly when not.
  const focus = (typeof th.focus === "string" && th.focus.trim())
    ? `<p class="th-focus">now: ${esc(th.focus.trim())}</p>` : "";
  const why = (typeof th.why === "string" && th.why.trim())
    ? `<div class="th-genesis th-why"><span class="rune">why</span><span>${esc(th.why.trim())}</span></div>` : "";
  ...
  setTopbar(`<a class="tb-back" href="#/initiatives"><span class="arw">←</span>Initiatives</a>`
...
// ---- initiatives: lifecycle-first rows ---------------------------------------
...
function evidenceRow(ev){
  ...
  const time = `<span class="ev-time">${fmtShortDate(ev.ts)}</span>`;
...
function threadRow(th, key, i){
  ...
  if(typeof th.focus === "string" && th.focus.trim()){
    hero += `<p class="tr-focus">now: ${esc(th.focus.trim())}</p>`;
  }
...
// the money panel: every dangling issue across the repo
function unfinishedPanel(items){
  ...
  return `<div class="unf-panel">`
    ...
    +   `<span class="unf-count">${items.length} dangling issue${items.length === 1 ? "" : "s"}</span>`
    +   `<span class="unf-sub">open branches &amp; shipped-but-open — needs a landing</span></div>`
    + `<div class="unf-list">${rows}</div></div>`;
}

// The open loose ends (deferred/followup events with no resolved_at): each row is the
// summary, its age, and the initiative it belongs to. Fed by /api/inflight's `loose` array.
function looseEndsSection(loose){
  if(!loose.length) return "";
  const rows = loose.map((ev, i) => {
    const owner = ev.thread_id != null
      ? `<a href="#/thread/${esc(ev.thread_id)}">${esc(ev.thread_title || "initiative")}</a>`
      : "";
    return `<div class="loose-row" data-key="loose-${esc(ev.id)}" `
      + `style="--hue:${hueVar(ev.type)};--d:${Math.min(i, 10) * 24}ms">`
      ...
      +     (owner ? `<span class="sep">·</span>${owner}` : "")
      +   `</div>`
      + `</div>`
      + `<span class="loose-age">${relAge(ev.ts)}</span>`
      + `</div>`;
  }).join("");
  return `<div class="section loose-section" id="looseEnds">`
    + `<div class="section-head"><h2>Loose ends</h2><span class="rule"></span>`
    + `<span class="n">${loose.length}</span></div>`
    + `<div class="loose-list">${rows}</div></div>`;
}

function initiativesControls(){
  const filterBtn = (f, label) =>
    `<button class="seg ${state.filter === f ? "on" : ""}" data-filter="${f}">${label}</button>`;
  return `<span class="tb-title">Initiatives</span>`
    + `<span class="tb-spacer"></span>`
    + `<div class="segmented filter">${filterBtn("all", "All")}${filterBtn("unfinished", "Unfinished")}</div>`
    + liveTag();
}

// Initiatives and Timeline share a two-column shell: the ledger on the left, the worktrees
// panel on the right. Built once per entry so the panel keeps its own state (open menus,
// focus) across the left column's repaints.
...
// The home view: one calm list of goal-first initiatives — unfinished panel on top,
// initiative rows in the middle, open loose ends at the bottom.
async function renderInitiatives(force){
  ensureWorkShell();
  renderWorktreePanel(force); // right rail — independent, runs concurrently

  const [threads, inflight] = await Promise.all([
    api("/api/threads"),
    api("/api/inflight"), // only its `loose` array is used here
  ]);
  const loose = inflight.loose || [];
  ...
  if(unfinishedItems.length) html += unfinishedPanel(unfinishedItems);
  ...
  html += list.length
    ...
    : emptyBlock(
        unfilter ? "Nothing unfinished" : "No initiatives yet",
        unfilter ? "every initiative here has landed" : "initiatives appear here as work streams in");
  html += looseEndsSection(loose);
  state.primed = true;

  setTopbar(initiativesControls());
  paint(html, { target: $("#workCol") });
  state.sig = sig;
  if(state.scrollToLoose){
    state.scrollToLoose = false;
    const el = document.getElementById("looseEnds");
    if(el) el.scrollIntoView({ behavior:"smooth", block:"start" });
  }
}

// The flat chronological ledger — the "what happened when" lens, on its own spine.
async function renderTimeline(force){
  ensureWorkShell();
  renderWorktreePanel(force); // right rail — independent, runs concurrently

  const timeline = await api("/api/timeline?days=3650&limit=500");
  const sig = "tl:" + timeline.length + ":" + (timeline[0] ? timeline[0].id : 0);
  if(!force && sig === state.sig) return;

  setTopbar(`<span class="tb-title">Timeline</span>`
    + `<span class="tb-spacer"></span>${liveTag()}`);
  paint(renderDays(timeline), { scrollSafe: !force, target: $("#workCol") });
  markSeen(timeline);
  state.sig = sig;
}
...
async function loadMeta(){
  try{
    ...
    $("#initiativesBadge").textContent = m.threads != null ? m.threads : "";
    $("#usageBadge").textContent = m.tokens ? fmtTokens(m.tokens) : "";
    // the count links to the Loose ends section on Initiatives (see initInteractions)
    $("#looseFoot").innerHTML = m.loose
      ? `<a class="loose-jump" href="#/initiatives"><span class="n">${m.loose}</span> loose end${m.loose===1?"":"s"} open</a>`
      : `no loose ends`;
...
async function selectProject(slug){
  ...
  renderMenu();
  await loadMeta();
  // A thread id belongs to the old project; drop back to Initiatives rather than 404.
  // Otherwise just re-render the current view against the new project.
  if(state.view === "thread"){ location.hash = "#/initiatives"; }
...
function initInteractions(){
  ...
  // the rail-foot loose-ends count jumps to the Loose ends section on Initiatives
  $("#looseFoot").addEventListener("click", (e) => {
    const a = e.target.closest("a.loose-jump");
    if(!a) return;
    if(state.view === "initiatives"){
      e.preventDefault();
      const el = document.getElementById("looseEnds");
      if(el) el.scrollIntoView({ behavior:"smooth", block:"start" });
    } else {
      state.scrollToLoose = true; // consumed after the Initiatives render
    }
  });
...
async function boot(){
  ...
  setInterval(() => {
    loadProjects();
    if(state.view === "initiatives" || state.view === "timeline") refresh(true);
  }, 60000);
}
