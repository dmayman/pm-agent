---
description: Set up pm-agent in this repo — enable the ledger, backfill the timeline, and point to the dashboard. One idempotent command.
argument-hint: "[--explicit]"
---
...
1. **Ledger** — enables the observational timeline (`.claude/pm-ledger.md` marker) and
   backfills it from the repo's git history (issues, commits, merged PRs — the GitHub
   parts need an authenticated `gh`, and are skipped cleanly without it).
2. **Dashboard** — points to `pm-agent serve`.
