---
description: Checkpoint a unit of work — PM commits the staged changes (issue-linked message) and records progress on the issue. Not closeout; doesn't change this session's role.
argument-hint: "<issue-number> [what this checkpoint covers]"
---
...
prompt, give it:
- The GitHub issue number (from the arguments below; infer from the session if absent).
...
1. Review the **staged** diff (not the whole working tree), write a commit message that
   references the issue (`#<n>` — a plain reference, NOT a `Closes`/`Fixes` keyword, so the
   issue isn't auto-closed at a mid-work checkpoint), and commit. It commits only what you
   staged — it must NOT `git add` anything more.
2. Record progress on the issue: add a comment summarizing what's now done (referencing the
   commit), ensure the `status:in-progress` label is set if it isn't already, and note the
   remaining work so completeness is visible at a glance.
