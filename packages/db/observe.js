...
1) GOALS — Identify the SET of durable goals this session advances or spawns. Treat the FIRST user message of the excerpt as a goal-candidate: the moment a goal is framed is often the opening prompt, and it must not slip by as ordinary chatter. For each goal decide whether it advances one of the existing initiatives (below) or is a NEW goal.
   ...
   TITLES — a title must be OUTCOME-SHAPED and derived from the goal: name the thing that will be TRUE when the arc is finished, not the surface it touches. NEVER title an initiative after a component, subsystem, feature area, or capability bucket — those are areas, not initiatives, and they read as vague and evergreen. GOOD: "Per-muscle recovery readiness signal", "Reviewable set_plan diffs", "Deterministic duplicate-row cleanup". BAD: "Coach Capabilities", "Coach Loop & Tools", "Operator Editing Tools", "Plan Model & Execution" — each of those is a surface/area, not a goal. Prefer a concrete noun phrase describing the outcome; if you catch yourself writing an "&"-joined area name or a generic bucket, rewrite it as the specific result the work delivers. Every NEW goal ("isNew": true) MUST get its OWN distinct, outcome-shaped short title. Do NOT file multiple new goals under a single existing umbrella/surface title — that silently collapses them back into one initiative. Reuse an EXACT existing title ONLY when "isNew": false, i.e. the goal genuinely advances that initiative. Example: in a retro that frames "reviewable set_plan diffs" AND "fix the deterministic duplicate-row projection", those are TWO separate new initiatives with two distinct outcome titles ("Reviewable set_plan diffs" and "Deterministic duplicate-row cleanup") — NOT two goals both titled "Plan Model & Execution".
...
Respond with ONLY a JSON object (no prose, no code fence) of this shape:
{
  "goals": [
    {
      "initiative": "the EXACT title of a matching initiative above, or a short new OUTCOME-SHAPED title (the result the work delivers, never a component/surface/area name) if none fits",
      "isNew": true or false,
